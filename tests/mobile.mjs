...
{
  /*
    The film shows through the headline because one element carries both a dark
    plate and white type and the group is composited with `multiply`. That only
    works while nothing between it and the film isolates blending — and a
    stacking context forces isolation, which is how the first attempt failed:
    `z-10` on the copy layer left the letters solid white, and every structural
    check still passed because the markup was correct.

    So this walks the ancestors and fails on anything that would isolate.
  */
  const k = await page.evaluate(() => {
    const h1 = document.querySelector("header h1");
    if (!h1) return null;
    const plate = h1.parentElement;
    const header = h1.closest("header");

    const isolates = (el) => {
      const c = getComputedStyle(el);
      const reasons = [];
      if (c.isolation === "isolate") reasons.push("isolation");
      if (c.zIndex !== "auto") reasons.push(`z-index:${c.zIndex}`);
      if (parseFloat(c.opacity) < 1) reasons.push(`opacity:${c.opacity}`);
      if (c.transform !== "none") reasons.push("transform");
      if (c.filter !== "none") reasons.push("filter");
      if (c.backdropFilter && c.backdropFilter !== "none") reasons.push("backdrop-filter");
      if (c.mixBlendMode !== "normal") reasons.push(`blend:${c.mixBlendMode}`);
      if (c.contain.includes("paint")) reasons.push("contain:paint");
      if (c.willChange !== "auto") reasons.push(`will-change:${c.willChange}`);
      return reasons;
    };

    const blockers = [];
    for (let el = plate.parentElement; el && el !== header; el = el.parentElement) {
      const r = isolates(el);
      if (r.length) blockers.push(`${el.tagName.toLowerCase()}.${(el.className || "").toString().split(" ")[0]}: ${r.join("+")}`);
    }

    return {
      blend: getComputedStyle(plate).mixBlendMode,
      colour: getComputedStyle(h1).color,
      weight: getComputedStyle(h1).fontWeight,
      plateHasBackground: getComputedStyle(plate).backgroundImage !== "none",
      headerIsolates: getComputedStyle(header).isolation === "isolate",
      blockers,
      videoBehind: Boolean(header.querySelector("video")),
    };
  });

  if (k) {
    ok(k.blend === "multiply", "the headline plate composites with multiply", k.blend);
    ok(k.plateHasBackground, "the headline plate carries its own dark gradient");
    ok(
      k.colour === "rgb(255, 255, 255)",
      "the headline type is pure white, so it multiplies the film by one",
      k.colour,
    );
    ok(Number(k.weight) >= 700, "the headline is heavy enough to show the film", k.weight);
    ok(k.videoBehind, "the film sits behind the headline");
    ok(k.headerIsolates, "the hero isolates the blend from the rest of the page");
    ok(
      k.blockers.length === 0,
      "nothing between the headline and the film isolates blending",
      k.blockers.join(" | "),
    );
  }
}
